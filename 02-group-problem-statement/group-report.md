...
```mermaid
flowchart TD
    A["Người dùng nói ngân sách tổng, hạn nhận và món muốn ăn"] --> B{"Đủ vị trí, ngân sách, thời gian?"}
    B -- "Thiếu" --> C["Hỏi đúng thông tin thiếu"]
    C --> A
    B -- "Đủ" --> D["Lấy menu, tổng giá, ETA, trạng thái món có nguồn"]
    D --> E{"Dữ liệu đủ tin cậy?"}
    E -- "Không" --> F["Báo chưa chắc; mở menu gốc, không đoán"]
    E -- "Có" --> G["Rule loại quán đóng, món hết, quá giá/hạn"]
    G --> H{"Có món đạt?"}
    H -- "Không" --> I["Xin phép nới một điều kiện"]
    I --> A
    H -- "Có" --> J["Gom món cũ, bộ lọc, gợi ý cá nhân nếu được phép"]
    J --> K["Bỏ trùng; xếp tối đa 3 món + lý do, tổng giá, ETA"]
    K --> L["Người dùng kiểm, tự chọn và tự đặt"]
```
